...
type CRDIdentifiersGoTemplateVars struct {
	APIVersion   string
	ServiceAlias string
	CRD          *model.CRD
}

func NewCRDIdentifiersGoTemplate(tplDir string) (*ttpl.Template, error) {
	tplPath := filepath.Join(tplDir, "pkg", "crd_identifiers.go.tpl")
	tplContents, err := ioutil.ReadFile(tplPath)
	if err != nil {
		return nil, err
	}
	t := ttpl.New("crd_identifiers")
