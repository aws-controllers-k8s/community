...
How bulk adoption works, what each flag means, and how the CLI is released as new
ACK resources land upstream. The CLI this command belongs to is described in
[ack-cli.md](ack-cli.md).
...
--tags "Environment=prod" --service eks --kind Nodegroup
  ...
  {"clusterName": "prod-cluster", "name": "ng-general"}
        │
        ▼  emit CR: adoption-fields + policy + region + provenance
```
...
`--debug` go to stderr, so the command pipes into `kubectl` cleanly. Use `kubectl
create` (POST) rather than `apply` (patch): `create` is atomic and lets the API
server reject a duplicate name with `AlreadyExists`, which is what prevents the
same AWS resource being adopted twice.
...
[adoption]: https://aws-controllers-k8s.github.io/community/docs/user-docs/adoption/
...
```
ack-cli adopt --service SERVICE --kind KIND --adoption-set NAME --tags "K=V ..." [flags]
```
...
| `--adoption-set NAME` | Names the collection this adopts; applied as the `ack.k8s.aws/adoption-set` label. Does **not** affect CR names. **Required** |
| `--policy` | `adopt` (default) or `adopt-readonly` |
| `--region` | AWS region. Defaults to the environment; fails if none resolves (§2.2) |
| `--namespace` | Namespace on emitted CRs |
| `-o, --output` | Write to a file instead of stdout. Refuses to overwrite |
...
| `--policy` | Emitted | Effect |
|------------|---------|--------|
| `adopt` (default) | `adoption-policy: adopt` | The controller finds the resource and takes over managing it |
| `adopt-readonly` | `adoption-policy: adopt` **+** `read-only: "true"` | Same discovery, but ACK never mutates the resource |

Read-only is a separate annotation rather than a policy value — that is how the
runtime models it, which is why both rows emit the same policy.

ACK's third policy, `adopt-or-create`, is **refused**, with an explanation rather
than a generic invalid-value error since a user could reasonably expect it to work:
it creates the resource from `spec` when one is not found, and these manifests
carry no spec (§3).

**Deletion policy.** Emitted CRs do not set `services.k8s.aws/deletion-policy`, so
ACK's default applies and `kubectl delete` on an adopted CR **deletes the real AWS
resource**. When adopting infrastructure ACK did not create, `--policy
adopt-readonly` is the safer first pass. To keep resources on CR deletion, annotate
after creating — the adoption-set label makes that one command:
...
metadata:
  ...
  namespace: default
  labels:
    ack.k8s.aws/adoption-set: platform-prod           # ours: select this set
  annotations:
    ack.k8s.aws/source-arn: arn:aws:eks:us-west-2:…   # ours: trace to the resource
    services.k8s.aws/adoption-fields: '{"clusterName":"prod-cluster","name":"ng-general"}'
    services.k8s.aws/adoption-policy: adopt
    services.k8s.aws/region: us-west-2
```

- **`spec` is omitted entirely**, never emitted as `spec: {}`. The controller fills
  it in from the live resource while adopting. The distinction matters: ACK's
  generated types declare `spec,omitempty`, so a CR with no spec validates, while
  `spec: {}` is present-but-empty and gets checked against spec's own required
  fields — which most kinds have — and the API server rejects it.
- **Keys we add live under `ack.k8s.aws/`**, separate from `services.k8s.aws/`, the
  runtime's own namespace, so a reader can tell which keys the controller acts on
  and which are bookkeeping.
- **`--adoption-set` is the handle for the collection.** It is a label, never part
  of a name, so one value can span several runs and still select them all — which is
  also how a user verifies the adoption took:
...
- **Names derive from resource identity** — the identifier values plus a digest of
  ...
  rejects it. This matters because ACK has no cross-CR ownership guard: two CRs
  naming one AWS resource would both reconcile it, and with the default deletion
  policy deleting either would delete the shared resource. The ARN digest is what
  separates same-named resources in different regions or accounts.
- **Resolve or refuse.** An ARN that does not match its kind's template is skipped
  with a reason on stderr and counted in the summary. A partial or guessed
  identifier map is never emitted, because it would adopt the wrong resource.
- **Output is deterministic.** `GetResources` is paginated and promises no
  particular order, so CRs are sorted by name before rendering and
  `adoption-fields` is key-sorted. The same inputs therefore produce byte-identical
  output, and re-running after new resources are tagged adopts exactly the delta —
  the CRs that already exist are rejected by name, the new ones are created.
...
(§4.1). That one form also covers what would otherwise be special cases: a key
holding the AWS account is `"${Account}"`, and a kind whose identifier *is* its ARN
is one binding `{"key": "arn", "from": "${ARN}"}`, where `${ARN}` stands for the
whole matched ARN.
...
`${RepositoryName}` placeholder binds to its `name` key. Matching by name rather
than by position is what makes the mapping independent of segment order: a wafv2 web
ACL's ARN is `${Scope}/webacl/${Name}/${Id}` while its ACK keys are
`[name, id, scope]`.
...
Composed bindings need a real-ARN test on top of the round-trip check (§5), since the
composition can be right in shape and wrong in detail — an IAM role with a path, or
an SSO role, does not render the same way.
...
**Validation is the gate.** Because every step must pass before publish, a bad
mapping fails the release rather than shipping — and since the generator is
deterministic, a chart tag that changes no resource produces an identical catalog
and a release differing only in version. The regenerated catalog lands in the
release commit, so any change to a binding is visible in that diff.

A new upstream resource therefore needs no work from us: the controller releases,
