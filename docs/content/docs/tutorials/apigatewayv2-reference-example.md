---
title: "Manage HTTP API using the ACK APIGatewayv2 Controller"
...
In this tutorial we will invoke a single public endpoint(https://httpbin.org/get) by fronting it with an HTTP API. You
can add multiple Routes and Integrations to the same API and invoke many endpoints using the single HTTP API endpoint.
HTTP API helps in creating frontend and organizing microservices using multiple Routes and Integrations.
...
[Amazon EKS Setup](https://docs.aws.amazon.com/deep-learning-containers/latest/devguide/deep-learning-containers-eks-setup.html).
...
You can deploy the ACK service controller for Amazon APIGatewayv2 using the [apigatewayv2-chart Helm chart](https://gallery.ecr.aws/aws-controllers-k8s/apigatewayv2-chart).
You can download it to your workspace using the following command:
...
You can now use the Helm chart to deploy the ACK service controller for Amazon APIGatewayv2 to your EKS cluster.
At a minimum, you need to specify the AWS Region for controller to execute the APIGatewayv2 API calls.

For example, to specify that the APIGatewayv2 API calls go to the `us-east-1` region, you can deploy the service
...
You can refer to [API Reference](https://aws-controllers-k8s.github.io/community/reference/) for *APIGatewayv2*
...
cat <<EOF > apigwv2-httpapi-using-ack.yaml
...
kubectl apply -f apigwv2-httpapi-using-ack.yaml
```

The manifest, created by the script above, contains 4 APIGatewayv2 custom resources i.e. API, Integration, Route and Stage.
...
You can view these custom resources using following commands:
```bash
...
resources. You can find the *ACK.ReferencesResolved* condition inside the *Status* of Route, Integration and Stage
...
You can refer to [API Reference](https://aws-controllers-k8s.github.io/community/reference/) for *APiGatewayv2* to find
...
You can remove all the resource created in this tutorial using `kubectl delete` command.

```bash
kubectl delete -f apigwv2-httpapi-using-ack.yaml
```
