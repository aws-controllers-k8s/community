...
def name_suffix():
    return random_suffix_name("model-explain-job-def", 32)

@pytest.fixture(scope="module")
def xgboost_churn_model_explainability_job_definition(name_suffix, xgboost_churn_endpoint):
    ...
    job_definition_reference, job_definition_data, resource = create_sagemaker_resource(
        ...
        spec_file="xgboost_churn_model_explainability_job_definition",
...
def get_sagemaker_model_explainability_job_definition(job_definition_name: str):
    try:
        return get_sagemaker_client().describe_model_explainability_job_definition(
...
class TestModelExplainabilityJobDefinition:
    def test_create_definition(self, xgboost_churn_model_explainability_job_definition):
        (job_definition_reference, resource) = xgboost_churn_model_explainability_job_definition
        assert k8s.get_resource_exists(job_definition_reference)
    
        job_definition_name = resource["spec"].get("jobDefinitionName")
        assert job_definition_name is not None

        description = get_sagemaker_model_explainability_job_definition(job_definition_name)
