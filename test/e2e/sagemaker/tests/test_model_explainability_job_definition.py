...
def _make_job_definition(endpoint_name):
    resource_name = random_suffix_name("model-explain-job-def", 32)
    ...
    data = load_resource_file(
        SERVICE_NAME, "xgboost_churn_model_explainability_job_definition", additional_replacements=replacements
    )
    logging.debug(data)
...
def get_sagemaker_model_explainability_job_definition(job_definition_name: str):
    try:
        hpo_desc = _sagemaker_client().describe_model_explainability_job_definition(
