...
def _make_job_definition(endpoint_name):
    resource_name = random_suffix_name("model-quality-job-definition", 32)
    ...
    data = load_resource_file(
        SERVICE_NAME, "xgboost_churn_model_quality_job_definition", additional_replacements=replacements
    )
    logging.debug(data)
